...
folder_paths.folder_names_and_paths["diffusion_models"] = (orig[0], {'.ckpt', '.pt', '.pt2', '.bin', '.pth', '.safetensors', '.pkl', '.sft', '.gguf'})
...
class FlowFluxPresetLoader:
    ...
    def clip_list():
        clips = []
        local_clips = folder_paths.get_filename_list("text_encoders")

        clips.append('None')

        for v in local_clips:
            clips.append(v)
    ...
    def load_flux(self, unet_name, weight_dtype, vae_name, clip_name1, clip_name2, clip_name3, clip_name4, base, hash, steps, guidance, sampler_name, scheduler, url):
        ...
        if (clip_name1 != "None"):
            ...
            print(f"[Flow Control] CLIP1 : {clip_name1}")

        if (clip_name2 != "None"):
            clip_path2 = folder_paths.get_full_path_or_raise("text_encoders", clip_name2)
            clip_paths.append(clip_path2)
            print(f"[Flow Control] CLIP2 : {clip_name2}")

        if (clip_name3 != "None"):
            clip_path3 = folder_paths.get_full_path_or_raise("text_encoders", clip_name3)
            clip_paths.append(clip_path3)
            print(f"[Flow Control] CLIP3 : {clip_name3}")

        if (clip_name4 != "None"):
            clip_path4 = folder_paths.get_full_path_or_raise("text_encoders", clip_name4)
            clip_paths.append(clip_path4)
            print(f"[Flow Control] CLIP4 : {clip_name4}")
